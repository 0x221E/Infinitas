...
namespace vm
{
    void ExecutorUtils::AssertStackSizeNotEqual(std::size_t size, const std::string& function, VMContext& context)
    {
    ...
    void ExecutorUtils::AssertStackSizeGreaterEqual(std::size_t size, const std::string& function, VMContext& context)
    {
        if (context.m_Stack.Size() >= size) return;
